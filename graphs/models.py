...
class chargedCarsWest(models.Model):
    ...
    Class the represents cars within the lot that are connected to EV chargers at West Parkade. 
        1. timestamp: Datetime field
        2. value: Number of cars that are connected and done charging (charged)
    """
    timestamp = models.DateTimeField(primary_key=True)
    value = models.IntegerField()

    def __str__(self):
        return "Timestamp: " +  self.timestamp.strftime("%m/%d/%y %H:%M:%S") + "Charged Cars: "+str(self.value)

class chargingCarsWest(models.Model):
    """
    Class the represents cars within the lot that are connected to EV chargers at West Parkade.
        1. timestamp: Datetime field
        2. value: Number of cars that are connected and still charging
    ...
    def __str__(self):
        return "Timestamp: " +  self.timestamp.strftime("%m/%d/%y %H:%M:%S") + "Cars Charging: "+str(self.value)
